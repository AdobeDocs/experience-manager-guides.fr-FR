...
Dans AEM Guides, vous pouvez ajouter et importer des citations et les appliquer à votre contenu. Vous pouvez ajouter ces citations à partir de n’importe quelle source de livres, de sites web et de revues.


AEM Guides vous aide à modifier, prévisualiser et trier vos citations. Après avoir ajouté vos citations dans le contenu, vous pouvez générer la sortie à l’aide du PDF natif. Vous pouvez également ajouter la page de bibliographie ou de références dans la sortie Native PDF.

AEM Guides prend en charge plusieurs styles de citations, tels que Modern Language Association (MLA), American Psychological Association (APA), Chicago, Institute for Electrical and Electronics Engineers (IEEE) et American Heart Association (AHA). Il est recommandé de les utiliser de manière claire et cohérente.
...
La boîte de dialogue **Ajouter une citation** s’ouvre.
   ...
   ![Panneau de citation dans l’éditeur web](images/citation-add.png) {width="300"}
...
Votre administrateur système peut modifier le style des citations à partir de la liste déroulante **Citations** dans l’onglet **Paramètres généraux** dans les **Paramètres de l’éditeur**.
...
Survolez le nom de la citation dans la liste. Sélectionnez ![](images/options.svg) icône **Options**.
...
Vous pouvez prévisualiser le contenu et le format de la citation dans le volet de prévisualisation.
   ...
   >[!NOTE]
   >
   >L’aperçu est basé sur le style de citation sélectionné par votre administrateur dans les **Paramètres de l’éditeur**.
...
1. Sélectionnez le ![](images/Delete_icon.svg) **Supprimer**.
...
Dans la sortie Native PDF, les citations apparaissent à l’endroit où vous les avez insérées. Vous pouvez également créer une page de bibliographie. Lorsque vous cliquez sur une citation, vous êtes redirigé vers la page de bibliographie.

Créez une mise en page **Citations** dans les modèles PDF et incluez-la dans votre document. Toutes les citations utilisées dans le livre sont répertoriées sur une page qui apparaît dans la sortie PDF. Pour en savoir plus sur la création d’une mise en page, voir [Créer une mise en page](/help/product-guide/native-pdf/components-pdf-template.md#create-page-layout).


Pour modifier l’affichage et la convivialité de la page de citation, consultez [Personnaliser les modèles PDF](/help/product-guide/native-pdf/pdf-template.md).
...
Pour en savoir plus sur les styles, consultez la vue [Utiliser les styles de contenu courants](/help/product-guide/native-pdf/stylesheet.md).
