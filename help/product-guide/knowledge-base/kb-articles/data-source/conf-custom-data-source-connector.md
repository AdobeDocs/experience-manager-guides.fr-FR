...
|---|---|---|
| getLogoUrl |  | <ul><li> Cette méthode renvoie l’URL utilisée comme logo du connecteur. <li> Par défaut, elle renvoie une chaîne vide, indiquant qu’aucune URL de logo n’est fournie, sauf si elle est remplacée. <br><b> Remarques supplémentaires </b> : <br> <ul><li> Si vous fournissez à la fois une URL de logo et un nom de classe de logo, l’URL de logo est utilisée pour afficher le logo dans l’interface utilisateur. <li> Si vous spécifiez l’URL du logo via les paramètres de configuration, elle remplace l’URL définie dans l’implémentation de la méthode . |
