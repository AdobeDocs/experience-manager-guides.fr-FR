...
| Nom de fichier/Nom de fichier PDF | Indiquez le nom du fichier avec lequel vous souhaitez enregistrer le PDF.<br><br>Vous pouvez également utiliser des variables lors de la définition du nom de fichier PDF. Pour plus d&#39;informations sur l&#39;utilisation des variables, consultez la section [Utiliser des variables pour définir les options Chemin de destination, Nom du site ou Nom de fichier](generate-output-use-variables.md#id18BUG70K05Z).<br><br>**Remarque** : si vous n&#39;indiquez pas de nom de fichier, le titre du plan DITA est utilisé pour générer le nom de fichier PDF final. Si le mappage n&#39;a pas de titre, le nom de fichier du mappage DITA est utilisé pour désigner le PDF final. Le nom du fichier est assaini à l’aide des règles configurées dans le système pour gérer tout caractère non valide. |
...
| Exécuter le workflow de post-génération | Lorsque vous sélectionnez cette option, une nouvelle liste déroulante Workflow de post-génération s’affiche, contenant tous les workflows configurés dans AEM. Vous devez sélectionner un workflow que vous souhaitez exécuter une fois le workflow de génération de sortie terminé. <br><br>**Remarque** : pour plus d’informations sur la création d’un workflow de génération post-sortie personnalisé, consultez la section Personnaliser le workflow de génération post-sortie dans Installer et configurer Adobe Experience Manager Guides as a Cloud Service. |
