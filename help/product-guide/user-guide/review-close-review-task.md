...
1. Ouvrez le **panneau de révision** dans le panneau de gauche de l’éditeur.
2. Sélectionnez la tâche de révision que vous souhaitez fermer ou soumettre à nouveau pour révision dans la liste **Tâches actives**.
...
5. Dans la boîte de dialogue **Mettre à jour la tâche**, choisissez l’une des actions suivantes :
   ...
   - **Demander un réexamen** : lance un autre cycle de révision. Vous pouvez sélectionner une autre version de la rubrique à réviser. Par défaut, la dernière version (ou la dernière version modifiée) du fichier de rubrique ou de mappage envoyé pour révision est sélectionnée. Vous pouvez également utiliser l&#39;option **Modifier les versions** pour définir la version des rubriques sélectionnées sur **Dernière version** ou **Ligne de base** selon vos besoins.  Les réviseurs et réviseuses qui ont terminé la révision précédente recevront une notification leur demandant de faire part de leurs commentaires sur la version mise à jour. Les autres réviseurs et réviseuses qui n’ont pas marqué la tâche de révision comme terminée sont avertis de la mise à jour de la rubrique.
