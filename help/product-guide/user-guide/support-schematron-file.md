...
## Définir des règles à l’aide du contexte text-node

Vous pouvez définir une règle Schematron avec un contexte text-node, par exemple `context="//text()"`, de sorte que la règle évalue directement les nœuds de texte au lieu d&#39;avoir à énumérer tous les éléments DITA possibles qui peuvent contenir ce texte.

Par exemple, la règle suivante marque les guillemets droits n&#39;importe où dans le texte de la rubrique :
...
Lorsque cette règle correspond, le résultat de la validation pointe vers le nœud de texte exact qui l’a déclenchée, plutôt que seulement vers l’élément englobant.

Les règles qui utilisent un contexte d’élément explicite, par exemple `context="//p"`, continuent à fonctionner comme avant et vous pouvez toujours utiliser l’une ou l’autre des approches selon la précision que vous souhaitez obtenir pour la correspondance et l’emplacement de l’erreur.
