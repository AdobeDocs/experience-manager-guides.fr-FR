...
| `payloadJson` | Objet JSON | Spécifiez les valeurs suivantes : -   `base` : chemin d’accès du dossier parent contenant la rubrique envoyée pour révision. <br> -   `asset` : chemin d’accès de la rubrique envoyée pour révision. <br> -   `referrer` : laissez ce champ vide. |
...
1. Utilisez le gestionnaire de modules pour télécharger le fichier `/libs/fmdita/mail`.
