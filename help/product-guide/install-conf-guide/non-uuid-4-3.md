...
Effectuez les étapes suivantes pour migrer votre contenu de la version 4.3.1 non-UUID vers la version 4.3.2 UUID.
...
Effectuez les vérifications suivantes sur les versions 4.3.1 non-UUID :
...
1. Vérifiez la compatibilité pour répertorier les informations suivantes :
   ...
   * Fichiers avec GUID filename
   ...
   ![onglet évaluation de la compatibilité dans la migration](assets/migration-compatibility-assessment-4-3-1.png)
...
1. Sélectionnez **Configurer les validations** dans le panneau de gauche. Ensuite, **Sélectionner un mappage** et **Sélectionner un paramètre prédéfini** du mappage pour les configurer. La liste actuelle de validation des sorties affiche les fichiers de sortie présents avant la migration et peut être validée par rapport aux fichiers de sortie générés ultérieurement après la migration.
   ...
   ![Onglet Configurer les validations dans la migration](assets/migration-configure-validation-4-3-1.png)
...
Pour plus d’informations, voir [purger les anciennes versions](../install-conf-guide/version-management.md#purge-older-versions-of-dita-files).
...
1. Installez la version UUID de la version prise en charge plutôt que la version non UUID. Par exemple, si vous utilisez la version 4.3.1 non-UUID, vous devez installer UUID version 4.3.2 [com.adobe.fmdita-6.5-uuid-4.3.2.1977.zip](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Faemdox%2Fother-packages%2Fuuid-migration%2F3-0%2Fcom.adobe.fmdita-6.5-uuid-4.3.2.1977.zip)) et exécuter la migration.


1. Installez le package de mise à niveau de la migration uuid [com.adobe.guides.uuid-upgrade-1.2.110.zip](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Faemdox%2Fother-packages%2Fuuid-migration%2F3-0%2Fcom.adobe.guides.uuid-upgrade-1.2.110.zip).
...
1. `http://<server-name>/libs/fmdita/clientlibs/xmleditor_uuid_upgrade/page.html` de lancement.
   ...
   ![Onglet Mise à niveau du système dans la migration](assets/migration-system-upgrade-4-3-1.png)
   >[!NOTE]
   >
...
![Onglet Référence et Révision dans la migration](assets/migration-baseline-review-upgrade-4-3-1.png)
...
|---|---|---|
| Nombre total de fichiers : 345997 | Nombre total de fichiers traités dans l’ensemble de dossiers donné. | S/O |
| Nombre de fichiers mis à niveau avec succès : 344516 | Nombre de fichiers migrés avec succès vers UUID. | S/O |
| Nombre de fichiers mis à jour avec des erreurs : 29 | Les erreurs qui se sont produites dans ces fichiers doivent être identiques à celles qui ont été signalées à l’étape de prémigration. | S/O |
| Nombre de fichiers ignorés : 1452 | Certains fichiers du référentiel de gestion des ressources numériques peuvent comporter des sous-ressources, lesquelles sont ignorées car elles ne sont pas éligibles à la migration UUID. | S/O |
...
| Temps total pris : 2:40:06,157 |  |  |

**Étape : Mettre à niveau les niveaux de référence**

| Résumé à l’issue du processus | Comment interpréter ? | Action |
|---|---|---|
| Nombre total de fichiers : 4833 | Nombre de plans DITA avec au moins 1 ligne de base. |  |
| Nombre de fichiers mis à niveau avec succès : 4705 | Nombre de plans DITA mis à niveau avec toutes les lignes de base. |  |
| Nombre de fichiers mis à jour avec des erreurs : 0 | Nombre de plans DITA dont les lignes de base n&#39;ont pas été mises à niveau. |  |
| Nombre de fichiers ignorés : 1647 | Nombre de plans DITA sans ligne de base |  |
| Nombre de fichiers n’ayant pas été mis à niveau : 128 | Le nombre d&#39;objets de référence qui n&#39;étaient pas valides (ils étaient vides) est répertorié dans le rapport (Excel). | Vérifier s&#39;il y a d&#39;autres erreurs que : `baselineObj not found on` |
...
1. Une fois la migration terminée, sélectionnez **Valider la mise à niveau du système** dans le panneau de gauche et validez les fichiers de sortie avant et après la migration pour vous assurer que la migration est réussie.
   ...
   ![Onglet Valider la mise à niveau du système dans la migration](assets/migration-validate-system-upgrade-4-3-1.png)
