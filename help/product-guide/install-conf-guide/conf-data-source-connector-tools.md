...
Les onglets suivants fournissent des instructions pour configurer un connecteur en fonction de votre configuration Experience Manager Guides : Cloud Service ou On-Premise.
...
Vous pouvez ajouter ou utiliser plusieurs ressources basées sur différentes URL pour certains connecteurs tels que Client REST générique, Salsify, Akeneo et les tableaux de développement Azure (ADO) Microsoft. Ensuite, contactez-les pour créer des fragments de contenu ou des rubriques à l’aide des générateurs qui les concernent.
...
**Installation d’un{#install-open-source-connector}** de connecteur open source
