---
title: Recommandations pour l’optimisation des performances d’On-Premise
...
**Quel est le changement ?**
Exclure `/var/dxml from oak:index/lucene`.

>[!NOTE]
>
> AEM Guides n’utilise jamais d’index Lucene pour rechercher du contenu dans `/var/dxml` nœud .

**Quand configurer ?**
Si vous apportez cette modification à un nouveau système avant de migrer le contenu, seule la mise à jour du `oak:index/lucene` est requise. Sinon, sur un système existant où le contenu est déjà migré, après avoir effectué la modification dans `oak:index/lucene,`, recréez les index pour Lucene \(*ce qui peut prendre quelques heures*\).

**Résultat de cette modification**
Cette modification empêche `/var/dxml` nœud d’être indexé et stocké dans le magasin de segments.
