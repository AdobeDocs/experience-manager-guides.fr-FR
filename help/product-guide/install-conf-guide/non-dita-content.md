...
### Convertir vos documents Word existants en document de type rubrique DITA

Les onglets suivants fournissent des instructions pour convertir vos documents Word existants en document de type rubrique DITA en fonction de votre configuration Experience Manager Guides : Cloud Service ou On-Premise.
...
1. Chargez les documents Word sources dans ce dossier. Pour plus d&#39;informations sur le chargement de contenu sur DAM, consultez [Charger du contenu DITA existant](upload-dita-content.md#).
...
D&#39;une manière générale, le processus de conversion nécessite que vous [Préparez les fichiers InDesign pour la conversion](aemg-appendix.md#id195DBF0045Z) et [Préparez le fichier de mappage pour la migration d&#39;InDesign vers DITA](aemg-appendix.md#id194AF0003HT), puis vous devez suivre la procédure donnée d&#39;exécution du processus de conversion.
...
1. Créez un nœud de recouvrement du dossier `config` dans le nœud `apps` .
   ...
   - Dans l&#39;élément `mapStyle`, spécifiez l&#39;emplacement du fichier de mappage contenant les mappages des styles de document InDesign aux éléments DITA. Le mappage par défaut est stocké dans le fichier situé à l’emplacement suivant :
     ...
     >[!NOTE]
     >
     > Pour plus d’informations sur la structure `stmap.adobeidml.xml` fichier et sur la personnalisation, voir [Annexe](aemg-appendix.md#id194AF0003HT).
...
1. Chargez les documents InDesign sources dans ce dossier. Pour plus d&#39;informations sur le chargement de contenu sur DAM, consultez [Charger du contenu DITA existant](upload-dita-content.md#).
...
### Convertir votre document XHTML existant en rubrique DITA

Les onglets suivants fournissent des instructions pour convertir votre document XHTML existant en rubrique DITA en fonction de votre configuration Experience Manager Guides : Cloud Service ou On-Premise.
...
1. Chargez les documents XHTML sources dans ce dossier. Pour plus d&#39;informations sur le chargement de contenu sur DAM, consultez [Charger du contenu DITA existant](upload-dita-content.md#).
...
Pour convertir du contenu Adobe FrameMaker non structuré (.fm et .book) en DITA structuré, vous pouvez utiliser le mécanisme de table de conversion de FrameMaker. Le processus se concentre sur l’évaluation du contenu existant, à l’aide d’une approche basée sur des modèles, et sur le mappage des styles FrameMaker à DITA par le biais de tableaux de conversion. Pour plus d’informations, consultez [Migration de la documentation technique de Unstructured vers DITA dans Adobe FrameMaker](https://migrate-from-unstructured-to-dita-step-by-step-guide.meetus.adobeevents.com/).

Après la conversion, le contenu structuré peut être migré dans AEM Guides. Pour plus d&#39;informations, consultez la section [Charger du contenu DITA existant à l&#39;aide de l&#39;outil WebDAV et de FrameMaker](./upload-dita-content.md).
...
Les onglets suivants fournissent des instructions pour migrer tout autre document structuré en fonction de votre configuration Experience Manager Guides : Cloud Service ou On-Premise.
...
1. Chargez les documents structurés source dans ce dossier. Pour plus d&#39;informations sur le chargement de contenu sur DAM, consultez [Charger du contenu DITA existant](upload-dita-content.md#).
